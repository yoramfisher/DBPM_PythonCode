...
cwd = os.getcwd()
def plot_Raster(dSN,vScan):

        svScan = str((vScan - 20) * -1)
        f = np.loadtxt(cwd + r"\raster" + str(vScan) + ".txt", dtype = float,comments="%", delimiter=None, skiprows = 5)
##### changed BWM because channel 1 is blowed up
        Ztot =  np.abs((np.array(f[:,2])))
        #Ztot =  np.abs((np.array(f[:,3])))
        Zmax = np.max(Ztot)
...
# Create the contour plot
        ...
        plt.title ("Device SN " + dSN +" " + svScan + " V" " 2D scan")
        plt.xlabel ("Position (mm)")
        plt.ylabel ("Position (mm)")
#plt.show()
        rpltName = "#F_RasterScan_SN" + dSN + svScan + ".jpg"
