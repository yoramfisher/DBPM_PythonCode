...
#code to turn out put on for Rigol supply
#:OUTP:STAT CH1,ON
#:OUTP:STAT CH1,Off
...
def ShowHelp():
   print( "Set a voltage. Works with the Rigol DP712.")
   print ("Usage:")
   # Using enumerate()
...
# 
def Rigol_ReadVoltage(port):
   resp = send(port, ":MEAS:VOLT:DC?", 1) # VOUT is actual voltage, not setpoint.
   return resp


def Rigol_SetVoltage(port,v):
   """ float v in c parlance
   """
   astring = ":APPLy {:.2f}".format(v)
   resp = send(port, astring, 1) # VOUT is actual voltage, not setpoint.
   return resp

def Rigol_tryConnect( port = thecomport):    
    """
...
def checkStatus(port, *argv):
   ...
   if resp != None:
      if resp.upper().startswith("RIGOL"):
         ret = 1
      print(kQUERY + ":" +resp)

   resp = sendCommand(port, "APPLy? CH1,VOLT", 1)
   if (resp != None):
      print("APPLy?" + ":" +resp)

   resp = sendCommand(port, ":MEAS:VOLT:DC?", 1)
   if (resp != None):
      print(":MEAS:VOLT:DC?" + ":" +resp)
...
def readVoltage(port,  *argv):
   ...
   resp = sendCommand(port, ":MEAS:VOLT:DC?", 1) # VOUT is actual voltage, not setpoint.
   print(resp)
   return resp
...
def setVoltage(port, *argv):
   """
   """
   sendCommands( port, [ ":APPLy " + argv[0][0] ], 0 )
